...
with tab3:
    ...
    if st.session_state.reanalyze_trigger:
        ...
        with st.spinner(f"🔍 {ticker_to_reanalyze} 재분석 중..."):
            ...
            if data and "Recommendation" in data and "❌ 분석 실패" not in data["Recommendation"]:
                ...
                st.error(f"❌ {ticker_to_reanalyze} 재분석 중 데이터 로드 실패. 잠시 후 다시 시도해주세요. 오류: {data.get('Recommendation', '알 수 없음')}")
    ...
    if not st.session_state.tickers and not st.session_state.default_tickers_loaded:
        default_tickers = ["OPTT", "APP", "INTC", "PLTR", "LAES", "TSSI"]
            ...
            for t in valid_tickers:
                ...
                momentum_data = {
                    ...
                    "값": [
                        ...
                        f"${data.get('Volume_Turnover_Million'):.2f}" if data.get('Volume_Turnover_Million') is not None else "N/A"
                    ]
                }
                ...
                market_pos_data = {
                    "지표": ["52주 고가 근접도(%)", "52주 저가 근접도(%)", "볼린저 상단", "볼린저 중간", "볼린저 하단", "현재가 BB 위치", "갭 상승률(%)", "3일 연속 마감"],
                    "값": [
                        f"{data.get('High_Proximity_Pct'):.2f}" if data.get('High_Proximity_Pct') is not None else "N/A",
                ...
                if current_price is not None and support_1st is not None and support_2nd is not None and support_3rd is not None:
                    if current_price >= support_1st:
                        st.success(f"**현재 가격 (${current_price:.2f})**은 1차 지지선 (${support_1st:.2f}) 위에 있습니다. 긍정적인 신호입니다.")
                    elif current_price >= support_2nd and current_price < support_1st:
                        st.warning(f"**현재 가격 (${current_price:.2f})**은 1차 지지선 (${support_1st:.2f}) 아래에 있지만, 2차 지지선 (${support_2nd:.2f}) 위에 있습니다. 2차 지지선에서의 반등을 기대할 수 있습니다.")
                    elif current_price >= support_3rd and current_price < support_2nd:
                        st.warning(f"**현재 가격 (${current_price:.2f})**은 2차 지지선 (${support_2nd:.2f}) 아래에 있지만, 3차 지지선 (${support_3rd:.2f}) 위에 있습니다. 장기 지지선에서의 반등을 기대할 수 있습니다.")
                    elif current_price < support_3rd:
                        st.error(f"**현재 가격 (${current_price:.2f})**은 3차 지지선 (${support_3rd:.2f}) 아래에 있습니다. 장기 추세 이탈 가능성이 있으니 매우 주의가 필요합니다.")
                    else:
